    ...
    while 1:
        ...
        for doc in docs:
            ...
            for i in range(len(doc["opens"])):
                past={}
                d0=datetime.today().strftime('%d/%m/%y')
                d1=datetime.fromtimestamp(doc["opens"][i]["d"]).strftime('%d/%m/%y')
                d0 = datetime.strptime(d0, '%d/%m/%y')
            ...
            if timeintervals:
                ...
                collection2.delete_one({"dmpEmId":doc['dmpEmId'],"day":doc['day']})
    ...
    if len(lst) > 1:
        ...
        if (len(mi) > 0):
            for k in range(len(mi)+1):
                if k == 0:
                    temp=list(filter(lambda i: i['t']<mi[k],dum))
                elif k == len(mi):
                    temp=list(filter(lambda i: i['t']>=mi[k-1],dum))
                else:
                    temp=list(filter(lambda i: i['t']>=mi[k-1] and i['t']<mi[k],dum))
                if(len(temp)>1):
                    result.append(temp)
            return result
        else:
